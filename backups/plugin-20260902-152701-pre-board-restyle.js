...
var LINE = LINE, LINE2 = LINE2, SBG = '#f1f2f3'
var ACC = '#0285ff', ACCD = '#0170dd', ACCS = '#e9f3ff'
var SURF = '#fff', HOV = HOV, FIELD = '#f2f2f3', INSET = INSET
...
function _installPwUiCss() {
  ...
  st.textContent = '[data-pw] .pw-hover{transition:background-color .15s}[data-pw] .pw-hover:hover{background:' + HOV + '}'
    ...
    + '[data-pw] .pw-board-col.is-dragover{box-shadow:inset 0 0 0 2px ' + ACC + '}'
  document.head.appendChild(st)
}
_installPwUiCss()
...
// 看板列（唯一实现）：列头 + 计数 + 空态/卡片列表 + 拖拽投放（拖拽高亮走 is-dragover class）
function BoardColumn(props) {
  var g = props.g, R = props.R, today = props.today, tasks = props.tasks
  return jsxs('div', { className: 'p-3 flex flex-col min-h-[80px] shrink-0 pw-board-col', style: { background: SURF, borderRadius: '10px', boxShadow: SH_CARD, minWidth: '230px', maxWidth: '300px' },
    ...
    children: [
      jsxs('div', { className: 'flex items-center gap-1.5 mb-2.5 px-1', style: { height: '26px' }, children: [
        ...
        jsx('span', { className: 'ml-auto text-[0.6875rem] tabular-nums', style: { color: MUT }, children: tasks.length }),
      ]}),
      jsx('div', { className: 'flex flex-col gap-2', children: tasks.length === 0
...
function BoardView(R) {
  ...
  return jsxs(Fragment, { children: [
    ...
    jsxs('div', { style: { position: 'relative', display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '14px', minHeight: '30px' }, children: [
      ...
      jsx('span', { onClick: hasFilters() ? clearAllFilters : undefined, className: 'select-none' + (hasFilters() ? ' pw-hover-faint' : ''), style: { height: '28px', lineHeight: '28px', padding: '0 10px', borderRadius: 99, fontSize: '0.6875rem', cursor: hasFilters() ? 'pointer' : 'default', color: hasFilters() ? MUT : FAINT, opacity: hasFilters() ? 1 : 0.55, transition: 'color .15s, background .15s' }, children: 'Clear' }),
