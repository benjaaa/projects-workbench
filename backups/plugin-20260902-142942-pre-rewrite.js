...
function RoundBtn(props) {
  return jsx('span', {
    ...
    className: 'inline-flex items-center justify-center cursor-pointer select-none shrink-0 transition-colors duration-150',
    ...
    onMouseEnter: function(e) { e.currentTarget.style.background = HOV; e.currentTarget.style.color = INK },
    onMouseLeave: function(e) { e.currentTarget.style.background = props.active ? HOV : 'transparent'; e.currentTarget.style.color = props.active ? INK : MUT },
...
function useReducer(initialView) {
  ...
  function doAddLog(t_) {
    ...
    runSpec({ op: 'add_log', path: t_.path, text: date + ' ' + text }).then(function() { tost('已添加推进记录') }).catch(function(e) {
      console.error('[pw] addLog failed:', e)
...
function ChatHome(R) {
  ...
  useEffect(function() {
    try {
      if (mascotRef.current && chatCardRef.current) {
        ...
        // 吉祥物容器顶部 = 卡上沿 - 8px（图案上移 2px）
        mascotRef.current.style.top = String(relTop - 37) + 'px'
      }
  ...
  function doSend() {
    ...
    console.log('[pw] doSend:', { text: rawText.slice(0, 30), projDir: projDir, cwd: cwd, ctx: ctxSel, attachPath: attachPath })
    setSending(true)
    ...
    host.request('session.create', { cwd: cwd || undefined, source: 'desktop', cols: 96 }).then(function(r) {
      console.log('[pw] session.create resp cwd:', r && r.info && r.info.cwd)
      ...
      if (cwd) {
        cwdSetP = host.request('session.cwd.set', { session_id: runtimeId || storedId, cwd: cwd })
          .then(function(sr) { console.log('[pw] session.cwd.set ok:', sr && sr.cwd) })
  ...
  return jsxs(Fragment, { children: [
    ...
    // 小吉祥物：春梢 + 电线杆（无动画，紧贴 chat 框上沿右侧；春梢在左、电线杆在右，同高）
    // 位置由 useEffect 测量主对话卡上沿后设置（不猜像素）
    jsx('div', {
      ref: mascotRef,
      style: { position: 'absolute', top: '60px', right: '14px', width: 'max-content', height: '40px', zIndex: 28, display: sending ? 'none' : 'flex', alignItems: 'flex-end', justifyContent: 'flex-start', opacity: sending ? 0 : 1, transition: 'opacity 0.25s ease' },
      children: jsx('div', { style: { display: 'flex', alignItems: 'flex-end', gap: '0px' }, children: [
        jsx('img', { key: 's', src: ASSET_DIR + encodeURIComponent('春梢停长末期.svg'), style: { width: '40px', height: '40px', display: 'block' } }),
        jsx('img', { key: 'p', src: ASSET_DIR + encodeURIComponent('电线杆.svg'), style: { width: 'auto', height: '40px', display: 'block', marginTop: '2px', marginLeft: '-4px' } }),
      ]}),
    }),
    ...
    jsxs('div', { ref: chatCardRef, style: { background: SURF, borderRadius: '16px', boxShadow: SH_CARD, padding: '16px 18px', textAlign: 'left', position: 'relative', zIndex: 30, transition: 'opacity 0.3s ease, background 0.3s ease', opacity: sending ? 0.55 : 1, background: sending ? '#f2f3f4' : SURF, pointerEvents: sending ? 'none' : 'auto' }, children: [
      ...
      atOpen && jsx('div', { ref: atPanelRef, style: { position: 'relative', zIndex: 30, borderTop: '1px solid ' + HOV, margin: '10px -4px 0', padding: '10px 4px 6px' }, children: jsxs('div', { children: [
        ...
        jsx('div', { style: { overflowY: 'auto', minHeight: 0, maxHeight: '250px' }, children: [
          ...
          atProjects.length === 0 ? jsx('div', { style: { padding: '4px 2px 10px', fontSize: '11.5px', color: FAINT }, children: '无匹配项目' }) : atProjects.map(function(p, pi) {
            ...
            return jsxs('div', { id: 'at-item-' + flatIdx, style: { display: 'flex', alignItems: 'center', gap: '8px', padding: '6px 2px', borderRadius: '6px', cursor: 'pointer', background: atIdx === flatIdx ? HOV : 'transparent' }, onMouseEnter: function(e) { e.currentTarget.style.background = HOV }, onMouseLeave: function(e) { e.currentTarget.style.background = atIdx === flatIdx ? HOV : 'transparent' }, onClick: function() { addCtx({ type: 'project', name: p.title, dir: p.dir || p.title, path: p.path }) }, children: [
          ...
          atInbox.length === 0 ? jsx('div', { style: { padding: '4px 2px 10px', fontSize: '11.5px', color: FAINT }, children: '无匹配收集' }) : atInbox.map(function(it, xi) {
            ...
            return jsxs('div', { id: 'at-item-' + flatIdx, style: { display: 'flex', alignItems: 'center', gap: '8px', padding: '6px 2px', borderRadius: '6px', cursor: 'pointer', background: atIdx === flatIdx ? HOV : 'transparent' }, onMouseEnter: function(e) { e.currentTarget.style.background = HOV }, onMouseLeave: function(e) { e.currentTarget.style.background = atIdx === flatIdx ? HOV : 'transparent' }, onClick: function() { addCtx({ type: 'inbox', name: it.title, dir: '', path: it.path, inbox_ts: it.ts }) }, children: [
      ...
      cmdPanel && jsx('div', { ref: cmdPanelRef, style: { position: 'relative', zIndex: 30, borderTop: '1px solid ' + HOV, margin: '10px -4px 0', padding: '10px 4px 6px' }, children: jsxs('div', { children: [
        ...
        jsx('div', { style: { overflowY: 'auto', minHeight: 0, maxHeight: '250px' }, children: [
          cmds.length === 0 ? jsx('div', { style: { padding: '14px', fontSize: '12px', color: FAINT }, children: '暂无指令，可在 2. Project/commands/ 下添加 .md 文件' }) : cmds.map(function(c) {
            return jsxs('div', { style: { display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 2px', borderRadius: '6px', cursor: 'pointer' }, onMouseEnter: function(e) { e.currentTarget.style.background = HOV }, onMouseLeave: function(e) { e.currentTarget.style.background = 'transparent' }, onClick: function() { pickCmd(c) }, children: [
      ...
      jsxs('div', { style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: '6px', paddingTop: '6px', borderTop: '1px solid ' + HOV, position: 'relative' }, children: [
        jsxs('div', { style: { display: 'flex', alignItems: 'center', gap: '6px' }, children: [
          // 单个 + 按钮（唤起附加菜单）
          jsx('span', { style: { display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: '22px', height: '22px', borderRadius: '7px', background: plusOpen ? SBG : INSET, color: BODY, fontSize: '15px', lineHeight: 1, cursor: 'pointer' }, onMouseEnter: function(e) { e.currentTarget.style.background = SBG }, onMouseLeave: function(e) { e.currentTarget.style.background = plusOpen ? SBG : INSET }, onClick: function() {
        ...
        plusOpen && jsxs('div', { 'data-plus-menu': '1', style: { position: 'absolute', bottom: '100%', left: 0, marginBottom: '8px', background: '#fff', borderRadius: '18px', boxShadow: '0 4px 24px rgba(0,0,0,0.12), 0 1px 4px rgba(0,0,0,0.06)', padding: '6px', minWidth: '240px', zIndex: 40, fontFamily: 'inherit' }, children: [
          // Add files or photos
          jsxs('div', { style: { display: 'flex', alignItems: 'center', gap: '10px', padding: '8px 10px', borderRadius: '10px', cursor: 'pointer' }, onMouseEnter: function(e) { e.currentTarget.style.background = HOV }, onMouseLeave: function(e) { e.currentTarget.style.background = 'transparent' }, onClick: function() { doPickFile() }, children: [
        ...
        plusOpen && plusSub === 'projects' && jsxs('div', { 'data-plus-menu': '1', style: { position: 'absolute', bottom: '100%', left: '250px', marginBottom: '8px', background: '#fff', borderRadius: '18px', boxShadow: '0 4px 24px rgba(0,0,0,0.12), 0 1px 4px rgba(0,0,0,0.06)', padding: '6px', width: '280px', zIndex: 41, fontFamily: 'inherit', maxHeight: '320px', overflowY: 'auto' }, children: [
          ...
          (R.ps || []).filter(function(p) { return p.status !== 'Done' && p.status !== 'Dropped' }).slice(0, 10).map(function(p) {
            return jsxs('div', { style: { display: 'flex', alignItems: 'center', gap: '10px', padding: '8px 10px', borderRadius: '10px', cursor: 'pointer' }, onMouseEnter: function(e) { e.currentTarget.style.background = HOV }, onMouseLeave: function(e) { e.currentTarget.style.background = 'transparent' }, onClick: function() { setPlusOpen(false); setPlusSub(null); addCtx({ type: 'project', name: p.title || p.dir, dir: p.dir || p.title, path: p.path }) }, children: [
        ...
        plusOpen && plusSub === 'cmds' && jsxs('div', { 'data-plus-menu': '1', style: { position: 'absolute', bottom: '100%', left: '250px', marginBottom: '8px', background: '#fff', borderRadius: '18px', boxShadow: '0 4px 24px rgba(0,0,0,0.12), 0 1px 4px rgba(0,0,0,0.06)', padding: '6px', width: '280px', zIndex: 41, fontFamily: 'inherit', maxHeight: '320px', overflowY: 'auto' }, children: [
          ...
          cmds.length === 0 ? jsx('div', { style: { padding: '10px', fontSize: '12px', color: FAINT }, children: '暂无指令' }) : cmds.map(function(c) {
            return jsxs('div', { style: { display: 'flex', alignItems: 'center', gap: '10px', padding: '8px 10px', borderRadius: '10px', cursor: 'pointer' }, onMouseEnter: function(e) { e.currentTarget.style.background = HOV }, onMouseLeave: function(e) { e.currentTarget.style.background = 'transparent' }, onClick: function() { setPlusOpen(false); setPlusSub(null); pickCmd(c) }, children: [
        ...
        plusOpen && plusSub === 'issues' && jsxs('div', { 'data-plus-menu': '1', style: { position: 'absolute', bottom: '100%', left: '250px', marginBottom: '8px', background: '#fff', borderRadius: '18px', boxShadow: '0 4px 24px rgba(0,0,0,0.12), 0 1px 4px rgba(0,0,0,0.06)', padding: '6px', width: '280px', zIndex: 41, fontFamily: 'inherit', maxHeight: '320px', overflowY: 'auto' }, children: [
          ...
          (R.ts || []).filter(function(t) { return t.status !== 'Done' && t.status !== 'Dropped' }).slice(0, 12).map(function(t) {
            return jsxs('div', { style: { display: 'flex', alignItems: 'center', gap: '10px', padding: '8px 10px', borderRadius: '10px', cursor: 'pointer' }, onMouseEnter: function(e) { e.currentTarget.style.background = HOV }, onMouseLeave: function(e) { e.currentTarget.style.background = 'transparent' }, onClick: function() { setPlusOpen(false); setPlusSub(null); addCtx({ type: 'issue', name: t.title, dir: t.dir, path: t.path, project: t.project, goal: t.goal, task_detail: t.task_detail, acceptance_criteria: t.acceptance_criteria }) }, children: [
...
// ─── Project List ────────────────────────────────────────
function ProjectListOld(R) {
  var flt = useState('active'), filter = flt[0], setFilter = flt[1]
  var shown = R.ps.filter(function(p) { return fltMatch(filter, p.status) })
  var view = useState('chat'), curView = view[0], setView = view[1]
  // 项目页内部 tab：'issues'(BoardView) / 'projects'(项目列表)——只切内容，不影响左右分页
  // 从详情返回时用 __pwReturnTab 恢复（默认 issues）
  var ct = useState(window.__pwReturnTab || 'issues'), contentTab = ct[0], setContentTab = ct[1]
  // 分页容器（Inbox ↔ Chat ↔ 项目页）：page 0=Inbox 1=Chat 2=项目页；默认 Chat，从详情返回时 __pwReturnPage=2 切到项目页
  var pageState = useState(window.__pwReturnPage || 1), page = pageState[0], setPage = pageState[1]
  // 消费返回标志，下次挂载恢复默认（chat 首页 + issues tab + page 0）
  window.__pwReturnPage = null; window.__pwReturnTab = null
  var scrollRef = useRef(null)
  function goPage(n) {
    setPage(n)
    if (scrollRef.current) scrollRef.current.scrollTo({ left: n * scrollRef.current.clientWidth, behavior: 'smooth' })
  }
  function onPagerScroll(e) {
    ...
    if (w > 0) {
      var p = Math.round(el.scrollLeft / w)
      if (p !== page) setPage(p)
    }
  }
  // 挂载时滚动容器到当前页（默认 Chat=1 居中；从详情返回时 page=2 项目页）
  // 用 setTimeout 等布局完成（clientWidth 挂载时可能为 0，直接设 scrollLeft 会停在页 0）
  useEffect(function() {
    setTimeout(function() {
      if (scrollRef.current) {
        scrollRef.current.scrollLeft = page * scrollRef.current.clientWidth
      }
    }, 50)
  }, [])
  // 去处理带回 chat：任务抽屉/Inbox 详情设 __pwGoChat 标记，已挂载的分页容器检测后切到 Chat 页
  useEffect(function() {
    if (window.__pwGoChat) {
      window.__pwGoChat = null
      setTimeout(function() { goPage(1) }, 0)
    }
  })
  // 共享 Header（无 Chat tab；contentTab 高亮 Issues/Projects）
  function fullHeader() {
    return jsxs('div', { className: 'flex items-end justify-between', style: { marginBottom: '28px' }, children: [
      jsxs('div', { children: [
        // Work Station 标题已去掉（仅保留 tab，布局不变）
        jsxs('div', { className: 'flex items-center gap-1', style: { marginTop: '10px' }, children: [
          jsx('span', { className: 'px-3 py-1 rounded-full cursor-pointer select-none text-[0.75rem] transition-all', style: { background: contentTab === 'issues' ? '#fff' : SEG_OFF, color: contentTab === 'issues' ? INK : MUT, fontWeight: contentTab === 'issues' ? 500 : 400, boxShadow: contentTab === 'issues' ? '0 1px 2px rgba(0,0,0,0.08)' : 'none' }, onClick: function() { setContentTab('issues') }, children: 'Issues ' + R.ts.length }),
          jsx('span', { className: 'px-3 py-1 rounded-full cursor-pointer select-none text-[0.75rem] transition-all', style: { background: contentTab === 'projects' ? '#fff' : SEG_OFF, color: contentTab === 'projects' ? INK : MUT, fontWeight: contentTab === 'projects' ? 500 : 400, boxShadow: contentTab === 'projects' ? '0 1px 2px rgba(0,0,0,0.08)' : 'none' }, onClick: function() { setContentTab('projects') }, children: 'Projects ' + R.ps.length }),
        ]}),
      ]}),
      jsx('span', { className: S.iconBtn, style: { color: MUT }, onClick: R.load, children: jsx(Codicon, { name: 'refresh', className: 'text-[0.9375rem]' }) }),
    ]})
  }
  // 项目页内容（Header + 内容）：contentTab 切换 Issues(BoardView)/Projects(项目列表)
  // 分页页 2 与完整视图共用，左右切换不受内部 tab 影响
    ...
    return jsxs('div', { children: [
      ...
      contentTab === 'projects'
        ? jsxs(Fragment, { children: [
            // 状态筛选 + 新建项目按钮
            jsxs('div', { className: 'flex items-center justify-between', style: { marginBottom: '14px' }, children: [
              jsx(StatusFilter, { value: filter, onChange: setFilter }),
              jsx(Btn, { onClick: function() { R.setMd('project') }, children: '+ New Project' }),
            ]}),
            // 项目列表
            R.loading ? jsx(Center, { icon: 'loading', spin: true, text: '加载中…' })
  ...
  // 分页容器：Inbox ↔ Chat 首页 ↔ 项目页（contentTab 在项目页内部切换，不影响分页），底部圆点 + 左右滑动
  // 页序：0=Inbox(左滑) 1=Chat(默认) 2=项目页(右滑)；初始 scrollLeft=1 页宽使 Chat 居中
  if (curView === 'chat') {
    return jsxs(Fragment, { children: [
      jsx('div', { 'data-pw': '1', className: S.page, style: { background: PAGE, overflow: 'hidden' }, children: jsxs('div', { className: S.wrap, style: { height: '100%', overflow: 'hidden', maxHeight: '100vh', padding: '0' }, children: [
        // 横向分页容器（scroll-snap 滑动切换；高度固定视口，子页各自纵向滚动，避免容器被内容撑高导致 snap 跳页）
        jsx('div', {
          ref: scrollRef,
          onScroll: onPagerScroll,
          style: { display: 'flex', overflowX: 'auto', scrollSnapType: 'x mandatory', WebkitOverflowScrolling: 'touch', scrollbarWidth: 'none', height: '100%', alignItems: 'stretch' },
          children: [
            // 页 0：Inbox（左滑进入；embedded 嵌入分页）
            jsx('div', { style: { width: '100%', flexShrink: 0, scrollSnapAlign: 'start', overflowY: 'auto', height: '100%' }, children: jsx(InboxList, Object.assign({}, R, { embedded: true })) }),
            // 页 1：Chat 首页（onExit: 'issues'/'board' 滑到项目页；compact 紧凑上移让圆点可见）
            jsx('div', { style: { width: '100%', flexShrink: 0, scrollSnapAlign: 'start', overflowY: 'auto', height: '100%' }, children: jsx(ChatHome, Object.assign({}, R, { compact: true, onExit: function(v) { if (v === 'board' || v === 'issues') goPage(2); else setView(v || 'board') } })) }),
            // 页 2：项目页（Header 的 Issues/Projects tab 只切内部内容，不影响左右分页）
            jsx('div', { style: { width: '100%', flexShrink: 0, scrollSnapAlign: 'start', overflowY: 'auto', height: '100%' }, children: projectPage() }),
          ],
        }),
        // 底部圆点指示器（固定视口底部，保持原样式；0=Inbox 1=Chat 2=项目）
        jsxs('div', { style: { position: 'fixed', bottom: '24px', left: '50%', transform: 'translateX(-50%)', display: 'flex', justifyContent: 'center', gap: '5px' }, children: [
          jsx('span', { style: { width: '5px', height: '5px', borderRadius: '99px', background: page === 0 ? DOT_ON : DOT_OFF, cursor: 'pointer', transition: 'background 0.2s ease' }, onClick: function() { goPage(0) } }),
          jsx('span', { style: { width: '5px', height: '5px', borderRadius: '99px', background: page === 1 ? DOT_ON : DOT_OFF, cursor: 'pointer', transition: 'background 0.2s ease' }, onClick: function() { goPage(1) } }),
          jsx('span', { style: { width: '5px', height: '5px', borderRadius: '99px', background: page === 2 ? DOT_ON : DOT_OFF, cursor: 'pointer', transition: 'background 0.2s ease' }, onClick: function() { goPage(2) } }),
        ]}),
      ]}) }),
      jsx(Modal, R),
      jsx(Drawer, R),
  ...
  // 完整视图（非 chat）：同一 projectPage（无圆点/滑动），由外部进入
  return jsxs(Fragment, { children: [
    jsx('div', { 'data-pw': '1', className: S.page, style: { background: PAGE }, children: jsxs('div', { className: S.wrap, children: [projectPage()] }) }),
    jsx(Modal, R),
    jsx(Drawer, R),
    jsx(Toast, R),
  ]})
...
// 默认过滤（用户确认 2026-08-31）：截止日期=近7天、状态=非已完成/取消
var IW_DEF_FILTERS = { status: ['open', 'In-Progress', 'Waiting', 'Agent', 'Review'], priority: [], handler: [], project: [], due: ['7d'] }
...
function BoardView(R) {
  ...
  // 视图（session 内有效）：'board' | 'list'，默认 List（用户确认 2026-08-31）
  var vst = useState('list'), view = vst[0], setView = vst[1]
  ...
  var groups = [
    ...
    { status: 'Agent', label: '托管给Agent' },
    { status: 'Review', label: '待人为确认' },
  ...
  function listView() {
    ...
    var bodyRows = listRows.map(function(t) {
      ...
      return jsxs('tr', {
        ...
        onMouseEnter: function(e) { e.currentTarget.style.background = HOV },
  ...
  // 看板视图：原 6 列渲染逻辑不变（仅改由过滤模型驱动）
  function boardView() {
    return jsx('div', { className: 'flex gap-3 overflow-x-auto', style: { paddingBottom: '14px' }, children: groups.map(function(g) {
        var gt = filtered.filter(function(x) { return (g.match || [g.status]).indexOf(x.status) >= 0 })
        return jsxs('div', { className: 'p-3 flex flex-col min-h-[80px] shrink-0', style: { background: SURF, borderRadius: '10px', boxShadow: SH_CARD, minWidth: '230px', maxWidth: '300px' },
          onDragOver: function(e) { e.preventDefault(); e.currentTarget.style.boxShadow = 'inset 0 0 0 2px ' + ACC },
          onDragLeave: function(e) { e.currentTarget.style.boxShadow = SH_CARD },
          onDrop: function(e) {
            e.preventDefault(); e.currentTarget.style.boxShadow = SH_CARD
            var path = e.dataTransfer.getData('text/plain')
            if (!path) return
            var t = (typeof filtered !== 'undefined' ? filtered : []).find(function(x) { return x.path === path })
            if (t && t.status !== g.status) R.doSetField(t, 'status', g.status, true)
          },
          children: [
          jsxs('div', { className: 'flex items-center gap-1.5 mb-2.5 px-1', style: { height: '26px' }, children: [
            dot(SDOT[g.status], 6),
            jsx('span', { className: 'text-[0.75rem] font-medium', style: { color: INK }, children: g.label }),
            jsx('span', { className: 'ml-auto text-[0.6875rem] tabular-nums', style: { color: MUT }, children: gt.length }),
          ]}),
          jsx('div', { className: 'flex flex-col gap-2', children: gt.length === 0
            ? jsx('div', { className: 'rounded-lg border border-dashed border-[#e0e2e5] py-5 text-center text-[0.6875rem]', style: { color: FAINT }, children: '—' })
            : gt.map(function(t) {
              return jsxs('div', {
                className: 'cursor-pointer transition-all duration-150 hover:bg-[#f7f8f9]',
                style: { borderRadius: '10px', padding: '10px 12px', background: '#fff', boxShadow: SH_CARD, borderLeft: dueFlag(t, today) ? '3px solid ' + DANGER : 'none' },
                draggable: true,
                ...
                onClick: function() { R.setDw(t) },
                children: [
                  // 行1：标题（列已按状态分类，无需状态chip）+ 重复标记
                  jsxs('div', { className: 'flex items-center gap-1', children: [
                    jsx('span', { className: 'text-[0.8125rem] font-medium truncate', style: { color: INK, flex: 1, minWidth: 0 }, children: t.title }),
                    t.repeat_mode ? jsx('span', { title: '重复周期任务：' + repLabel(t), style: { color: MUT, flexShrink: 0, fontSize: '11px' }, children: '🔁' }) : null,
                  ]}),
                  // 行2：优先级 + 截止/完成日期
                  jsxs('div', { className: 'flex items-center gap-1.5 mt-1.5', children: [
                    ...
                    (t.status === 'Done' || t.status === 'Dropped') && t.complete
                      ...
                      : t.due && jsxs('span', { className: 'flex items-center gap-0.5 text-[0.6875rem] tabular-nums', style: { color: MUT }, children: [
                          jsx(Codicon, { name: 'calendar', className: 'text-[0.75rem]' }),
                          jsx('span', { children: dL(t.due) }),
                        ]}),
                  ]}),
                  // 行3：处理人 + 项目名称（项目名靠右最多7字符）
                  t.handler || t.project ? jsxs('div', { className: 'flex items-center gap-1 mt-1.5', children: [
                    ...
                    jsx('span', { className: 'text-[0.6875rem]', style: { marginLeft: 'auto', color: FAINT, maxWidth: '7em', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }, children: (t.project || t.dir || '') }),
                  ]}) : null,
                ],
              }, t.path)
            }),
  ...
  return jsxs(Fragment, { children: [
    ...
    jsxs('div', { style: { position: 'relative', display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '14px', minHeight: '30px' }, children: [
      ...
      jsxs('div', { className: 'inline-flex items-center', style: hasFilters()
        ...
        hasFilters() ? defs.map(function(f) {
          ...
          return jsxs('span', { key: f.k, className: 'inline-flex items-center shrink-0', style: { gap: '4px', height: '22px', padding: '0 4px 0 8px', borderRadius: 99, fontSize: '0.75rem', whiteSpace: 'nowrap' }, children: [
            ...
            jsx('span', { title: '移除该条件', onClick: function() { clearField(f.k) }, className: 'cursor-pointer select-none', style: { width: '14px', height: '14px', lineHeight: '14px', borderRadius: 99, textAlign: 'center', color: FAINT, fontSize: '0.6875rem' }, onMouseEnter: function(e) { e.currentTarget.style.background = FIELD; e.currentTarget.style.color = INK }, onMouseLeave: function(e) { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = FAINT }, children: '✕' }),
          ] }, f.k)
        }) : jsx('span', { children: '暂无筛选条件' }),
        hasFilters() ? jsx('span', { title: '添加筛选', onClick: function() { setMenuOpen(!menuOpen); setOpenSub(null) }, className: 'cursor-pointer select-none inline-flex items-center justify-center shrink-0', style: { width: '20px', height: '20px', borderRadius: 99, color: MUT, fontSize: '0.875rem', lineHeight: 1 }, onMouseEnter: function(e) { e.currentTarget.style.background = FIELD; e.currentTarget.style.color = INK }, onMouseLeave: function(e) { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = MUT }, children: '+' }) : null,
      ]}),
      // Clear 跟随在筛选胶囊右侧（用户确认 2026-08-31）
      jsx('span', { onClick: hasFilters() ? clearAllFilters : undefined, className: 'select-none', style: { height: '28px', lineHeight: '28px', padding: '0 10px', borderRadius: 99, fontSize: '0.6875rem', cursor: hasFilters() ? 'pointer' : 'default', color: hasFilters() ? MUT : FAINT, opacity: hasFilters() ? 1 : 0.55, transition: 'color .15s, background .15s' }, onMouseEnter: function(e) { if (hasFilters()) { e.currentTarget.style.color = INK; e.currentTarget.style.background = HOV } }, onMouseLeave: function(e) { e.currentTarget.style.color = hasFilters() ? MUT : FAINT; e.currentTarget.style.background = 'transparent' }, children: 'Clear' }),
      jsx('span', { className: 'flex-1' }),
      // 圆按钮原语：筛选菜单 + 视图切换（图标显示当前视图，点击切换）
      ...
      // 筛选菜单弹层：一级字段 → hover 展开二级可选值（多选，勾选即时生效）
      menuOpen && jsxs(Fragment, { children: [
        ...
        jsx('div', { style: { position: 'absolute', top: '100%', left: 0, zIndex: 50, marginTop: '6px', background: '#fff', border: '1px solid ' + LINE2, borderRadius: '12px', boxShadow: SH_RAISED, padding: '5px', minWidth: '170px' }, onMouseLeave: function() { setOpenSub(null) }, children:
          defs.map(function(f) {
            var sel = filters[f.k] || []
            return jsxs('div', { key: f.k, style: { position: 'relative' }, onMouseEnter: function() { setOpenSub(f.k) }, children: [
              ...
              openSub === f.k && jsx('div', { style: { position: 'absolute', left: '100%', top: '-5px', marginLeft: '2px', background: '#fff', border: '1px solid ' + LINE2, borderRadius: '12px', boxShadow: SH_RAISED, padding: '5px', minWidth: '150px', maxHeight: '260px', overflowY: 'auto' }, children:
  ...
  return jsxs(Fragment, { children: [
    jsx('div', { 'data-pw': '1', className: S.page, style: { background: PAGE }, children: jsxs('div', { className: S.wrap, children: [
        ...
        jsxs('div', { style: { display: 'flex', flexDirection: 'column', gap: '8px' }, children: [
          ...
          jsxs('div', { style: { overflow: 'hidden', borderRadius: '10px', background: SURF, boxShadow: SH_CARD }, children: [
            ...
            jsxs('div', { style: { display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 12px', borderBottom: '1px solid ' + LINE }, children: [
              ...
              jsx('span', { style: { fontSize: '11px', color: FAINT, cursor: 'pointer', flexShrink: 0, transition: 'color 150ms' }, onMouseEnter: function(e) { e.currentTarget.style.color = ACC }, onMouseLeave: function(e) { e.currentTarget.style.color = FAINT }, onClick: function() { R.setEf('bg') }, children: '编辑' }),
            ]}),
            R.ef === 'bg'
          ...
          jsxs('div', { style: { overflow: 'hidden', borderRadius: '10px', background: SURF, boxShadow: SH_CARD }, children: [
            ...
            jsxs('div', { style: { display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 12px', borderBottom: '1px solid ' + LINE }, children: [
              ...
              jsx('span', { style: { fontSize: '11px', color: FAINT, cursor: 'pointer', flexShrink: 0, transition: 'color 150ms' }, onMouseEnter: function(e) { e.currentTarget.style.color = ACC }, onMouseLeave: function(e) { e.currentTarget.style.color = FAINT }, onClick: function() { R.setEf('goal') }, children: '编辑' }),
            ]}),
            R.ef === 'goal'
              ? jsxs('div', { style: { padding: '12px 12px 12px 12px' }, children: [
      ...
      jsxs('div', { style: { marginTop: '20px' }, children: [
        ...
        p_ts.length === 0
          ? jsx('div', { className: 'flex gap-3 overflow-x-auto', style: { paddingBottom: '14px' }, children: groups.map(function(g) {
              return jsx('div', { className: 'p-3 flex flex-col min-h-[120px] shrink-0', style: { background: SURF, borderRadius: '10px', boxShadow: SH_CARD, minWidth: '230px', maxWidth: '300px' }, children: [
                jsxs('div', { className: 'flex items-center gap-1.5 mb-2.5 px-1', style: { height: '26px' }, children: [
                  dot(SDOT[g.status], 6),
                  jsx('span', { className: 'text-[0.75rem] font-medium', style: { color: INK }, children: g.label }),
                  jsx('span', { className: 'ml-auto text-[0.6875rem] tabular-nums', style: { color: MUT }, children: '0' }),
                ]}),
                jsx('div', { className: 'rounded-lg border border-dashed py-5 text-center text-[0.6875rem]', style: { color: FAINT, borderColor: LINE2 }, children: '—' }),
              ]}, g.status)
            }) })
          : jsx('div', { className: 'flex gap-3 overflow-x-auto', style: { paddingBottom: '14px' }, children: groups.map(function(g) {
          ...
          onDrop: function(e) {
            ...
            var path = e.dataTransfer.getData('text/plain')
            if (!path) return
            var t = (typeof filtered !== 'undefined' ? filtered : p_ts).find(function(x) { return x.path === path })
            if (t && t.status !== g.status) R.doSetField(t, 'status', g.status, true)
          },
          children: [
                // 列头：圆点 + 标题 + 计数
                ...
                jsx('div', { className: 'flex flex-col gap-2', children: gt.length === 0
                  ...
                  : gt.map(function(t) {
                    return jsxs('div', {
                      ...
                      onClick: function() { R.setDw(t) },
                      children: [
                        // 行1：标题（列已按状态分类，无需状态chip）
                        jsx('div', { className: 'text-[0.8125rem] font-medium truncate', style: { color: INK }, children: t.title }),
                        // 行2：优先级 + 截止 + 产出数
                        jsxs('div', { className: 'flex items-center gap-1.5 mt-1.5', children: [
                          buiChip(PBGC[t.priority] ? PBGC[t.priority].color : MUT, PBGC[t.priority] ? PBGC[t.priority].background : HOV, PR[t.priority] || t.priority),
                          t.due && jsxs('span', { className: 'flex items-center gap-0.5 text-[0.6875rem] tabular-nums', style: { color: MUT }, children: [
                          ...
                          (t.output || []).length > 0 && jsxs('span', { className: 'flex items-center gap-0.5 text-[0.6875rem]', style: { color: MUT }, children: [
                            jsx(Codicon, { name: 'file', className: 'text-[0.75rem]' }),
                            jsx('span', { children: (t.output || []).length }),
                          ]}),
                        ]}),
                        // 行3：处理人
                        t.handler ? jsxs('div', { className: 'flex items-center gap-1 mt-1.5', children: [
                          jsx(Codicon, { name: 'account', className: 'text-[0.75rem]', style: { color: MUT } }),
                          jsx('span', { className: 'text-[0.6875rem]', style: { color: MUT }, children: t.handler }),
                        ]}) : null,
                      ],
              ...
              jsx('div', { className: 'overflow-hidden rounded-[10px] bg-surface', style: { boxShadow: SH_CARD, borderRadius: '10px', background: SURF, overflow: 'hidden' }, children: jsxs('div', { className: 'min-w-[420px]', style: { minWidth: '420px' }, children: [
                ...
                sessShown.map(function(s) {
                  ...
                  return jsxs('div', {
                    ...
                    onMouseEnter: function(e) { e.currentTarget.style.backgroundColor = HOV },
  ...
  return jsxs(Fragment, { children: [
    jsx('div', { 'data-pw': '1', className: S.page, style: { background: PAGE }, children: jsxs('div', { className: S.wrap, children: [
      jsxs('div', { className: 'flex items-baseline gap-3 mb-8', children: [
        jsx('span', { className: 'text-[0.8125rem] cursor-pointer transition-colors', style: { color: MUT }, onClick: function() { window.__pwReturnPage = 2; window.__pwReturnTab = 'projects'; R.setVw('projects') }, children: '← 项目' }),
        jsx('span', { className: S.h1, style: { color: INK }, children: 'All Issues' }),
        jsx('span', { className: S.mut, style: { color: MUT }, children: R.ts.length + ' 个' }),
      ]}),
      R.ts.length === 0
        ? jsx(Center, { icon: 'checklist', text: 'No issues yet' })
        : jsxs('div', { className: 'overflow-hidden rounded-[10px] bg-surface', style: { boxShadow: SH_CARD }, children: [
            // 表头（Filter Table）
            jsxs('div', { className: 'flex items-center gap-3 px-3 py-2 text-[0.6875rem] font-medium uppercase tracking-wide', style: { color: MUT, borderBottom: '1px solid ' + LINE }, children: [
              jsx('span', { className: 'w-4 shrink-0' }),
              jsx('span', { className: 'flex-1', children: 'Issue' }),
              jsx('span', { className: 'w-32 shrink-0', children: '所属项目' }),
              jsx('span', { className: 'w-16 shrink-0 text-right', children: '状态' }),
            ]}),
            R.ts.map(function(t) {
              var pn = R.pj(t.project || t.dir)
              var ch = (t.title || '?').charAt(0)
              return jsxs('div', {
                className: 'flex items-center gap-3 px-3 py-2.5 cursor-pointer transition-colors duration-100 hover:bg-[#f7f8f9]',
                style: { borderBottom: '1px solid ' + LINE },
                onClick: function() { R.setSel(t.project || t.dir); R.setVw('project'); R.setDw(t) },
                children: [
                  // 首字母徽章（Records Table 风格）
                  jsx('span', { className: 'flex items-center justify-center w-[22px] h-[22px] rounded-full shrink-0', style: { background: SBGC[t.status] ? SBGC[t.status].background : HOV, color: SBGC[t.status] ? SBGC[t.status].color : MUT, fontSize: '11px', fontWeight: 600 }, children: ch }),
                  jsx('span', { className: 'flex-1 min-w-0 text-[0.8125rem] font-medium truncate', style: { color: INK }, children: t.title }),
                  jsx('span', { className: 'w-32 shrink-0 text-[0.75rem] truncate', style: { color: MUT }, children: pn ? pn.title : (t.project || t.dir) }),
                  jsxs('span', { className: 'w-16 shrink-0 flex justify-end', children: [
                    buiChip(SBGC[t.status] ? SBGC[t.status].color : MUT, SBGC[t.status] ? SBGC[t.status].background : HOV, ST[t.status] || t.status),
                  ]}),
                ],
              }, t.path)
            }),
    ...
    return jsxs(Fragment, { children: [
      jsx('div', { style: { maxWidth: '720px', margin: '0 auto', paddingTop: '56px' }, children: [
        ...
        loading ? jsx(Center, { icon: 'loading', spin: true, text: '加载中…' })
          ...
          : jsx('div', { className: 'grid gap-2.5', style: { gridTemplateColumns: 'repeat(3, 1fr)' }, children: items.map(function(it) {
              var tsTxt = it.ts || ''
...
// ─── Task Drawer (右侧滑出，左右分栏) ────────────────────
function Drawer(R) {
  if (!R.dw) return null
  var t = R.dw
  // Inbox 详情用独立视图（InboxDetail），不渲染任务抽屉
  if (t && t.kind === 'inbox') return null
  // ── Inbox 添加抽屉（小灯泡/新收集唤起）──
  if (t && t.kind === 'inbox-add') {
    var iTitle = useState(''), iTitleV = iTitle[0], setITitle = iTitle[1]
    var iBody = useState(''), iBodyV = iBody[0], setIBody = iBody[1]
    function saveInbox() {
    ...
    return jsxs('div', { className: 'fixed inset-y-0 right-0 w-[380px] z-50 flex flex-col bg-surface', style: { boxShadow: '-8px 0 24px rgba(0,0,0,0.08)', borderLeft: '1px solid ' + LINE }, children: [
      // 头部：标题 + 右上角"查看所有"
      jsxs('div', { className: 'flex items-center justify-between px-5 py-4', style: { borderBottom: '1px solid ' + LINE }, children: [
        jsx('span', { className: 'text-[0.9375rem] font-semibold', style: { color: INK }, children: '收集灵感' }),
        jsxs('div', { className: 'flex items-center gap-3', children: [
          jsx('span', { className: 'text-[0.75rem] cursor-pointer', style: { color: '#2d6cdf' }, onClick: function() { R.setDw(null); R.setVw('inbox') }, children: '查看所有 →' }),
          jsx('span', { className: 'text-[0.8125rem] cursor-pointer', style: { color: MUT }, onClick: function() { R.setDw(null) }, children: '✕' }),
        ]}),
      ]}),
      // 表单
      jsxs('div', { className: 'flex-1 overflow-y-auto px-5 py-4 flex flex-col gap-4', children: [
        jsxs('div', { children: [
          jsx('div', { className: 'text-[0.6875rem] font-medium mb-1', style: { color: MUT }, children: '标题' }),
          jsx('input', { id: 'inboxTitle', type: 'text', placeholder: '想收集什么？', style: { width: '100%', border: '1px solid ' + LINE, borderRadius: '8px', padding: '8px 10px', fontSize: '0.8125rem', outline: 'none', background: '#fff' } }),
        ]}),
        jsxs('div', { children: [
          jsx('div', { className: 'text-[0.6875rem] font-medium mb-1', style: { color: MUT }, children: '正文' }),
          jsx('textarea', { id: 'inboxBody', placeholder: '补充细节…（可留空）', rows: 8, style: { width: '100%', border: '1px solid ' + LINE, borderRadius: '8px', padding: '8px 10px', fontSize: '0.8125rem', outline: 'none', background: '#fff', resize: 'vertical', lineHeight: 1.6 } }),
        ]}),
      ]}),
      // 底部保存
      jsxs('div', { className: 'px-5 py-3', style: { borderTop: '1px solid ' + LINE }, children: [
        jsx('button', { style: { width: '100%', fontSize: '0.8125rem', fontWeight: 600, color: '#fff', background: ORANGE, border: 'none', borderRadius: '8px', padding: '9px 0', cursor: 'pointer' }, onClick: saveInbox, children: '保存' }),
      ]}),
    ]})
  }
  ...
  var tdText = useState(''), tdVal = tdText[0], setTdVal = tdText[1]
  function startTdEdit() { setTdVal(t.task_detail || ''); setTdEditing(true) }
  function saveTd() {
  ...
  var acItems = (t.acceptance_criteria || []).map(function(a, i) {
    ...
    function doToggle() {
      ...
      // 乐观更新：改写 t.acceptance_criteria[i]（与 map 中 a 同引用），再强制重渲染
    ...
    // 圆形状态标记（morphing）：未开始=空心圈 / 通过=绿底✓ / 未通过=红底✕
    var mkBg = d ? GREEN : fl ? RED : 'transparent'
    var mkBd = d ? GREEN : fl ? RED : IN_BRD
    var mkFg = d ? '#fff' : fl ? '#fff' : 'transparent'
    return jsxs('div', {
      className: 'flex items-center gap-2.5 px-3 py-2 text-[0.75rem] transition-colors hover:bg-[#f4f5f6]',
      style: { borderBottom: i === acLen - 1 ? 'none' : '1px solid ' + LINE },
      children: [
        jsx('span', {
          ...
          style: {
            ...
            border: '1.5px solid ' + mkBd, background: mkBg, color: mkFg,
            fontSize: '0.625rem', lineHeight: '1', fontWeight: 700,
          },
          onClick: doToggle,
          children: d ? '✓' : fl ? '✕' : '',
        }),
        jsx('span', { className: 'flex-1 min-w-0 break-words', style: { color: fl ? DANGER : d ? MUT : BODY, textDecoration: d ? 'line-through' : 'none' }, children: txt }),
        jsx('span', { className: 'shrink-0 text-[0.625rem] font-medium', style: { color: d ? GREEN : fl ? RED : FAINT }, children: d ? '已通过' : fl ? '未通过' : '未开始' }),
      ],
    }, i)
  })
  // 推进记录编辑状态（记录 index 或 -1）
  ...
  function saveLogEdit(i) {
    ...
    if (v) {
      setForce(force + 1)
      editLog(t.path, i, v).catch(function(e) { console.error('[pw] editLog failed:', e); R.load() })
    }
    setLogEdit(-1)
  }
  // 任务关联的会话列表（从 frontmatter session_ids 查询）
  var ssF = useState([]), taskSess = ssF[0], setTaskSess = ssF[1]
  var slF = useState(false), sessLoaded = slF[0], setSessLoaded = slF[1]
  function loadTaskSess() {
    var ids = (t.session_ids || '').split(',').filter(Boolean)
    if (!ids.length) { setTaskSess([]); setSessLoaded(true); return }
    runSpec({ op: 'ops_session_by_ids', ids: ids.join(',') }).then(function(r) {
      setTaskSess(r.sessions || []); setSessLoaded(true)
    }).catch(function(e) { console.error('[pw] loadTaskSess failed:', e); setTaskSess([]); setSessLoaded(true) })
  }
  useEffect(function() { setSessLoaded(false); loadTaskSess() }, [t.path, t.session_ids])

  var logItems = (t.logs || []).map(function(l, i) {
    if (logEdit === i) {
      return jsxs('div', { style: { display: 'flex', alignItems: 'center', gap: '6px', padding: '3px 0' }, children: [
        jsx('span', { style: { color: MUT, fontSize: '0.625rem', fontFamily: 'monospace', width: '80px', flexShrink: 0 }, children: l.date }),
        jsx('input', {
          id: 'logEditInput' + i, defaultValue: l.text,
          style: { flex: 1, color: INK, height: '26px', border: '1px solid ' + IN_BRD, borderRadius: '5px', padding: '0 6px', fontSize: '0.75rem', outline: 'none', minWidth: 0, background: '#fff' },
          autoFocus: true,
          onFocus: function(e) { e.target.style.borderColor = FOCUS_BRD },
          onBlur: function(e) { e.target.style.borderColor = IN_BRD },
          onKeyDown: function(ev) { if (ev.key === 'Enter') saveLogEdit(i) },
        }),
        jsx('span', { style: { color: ACC, fontSize: '0.6875rem', cursor: 'pointer', whiteSpace: 'nowrap' }, onMouseDown: function(e) { e.preventDefault() }, onClick: function() { saveLogEdit(i) }, children: '保存' }),
      ]}, 'le' + i)
    }
    return jsxs('div', { style: { display: 'flex', alignItems: 'flex-start', gap: '6px', padding: '3px 0' }, children: [
      jsx('span', { style: { color: MUT, fontSize: '0.625rem', fontFamily: 'monospace', width: '80px', flexShrink: 0, lineHeight: '18px' }, children: l.date }),
      jsx('div', {
        style: { flex: 1, color: BODY, fontSize: '0.75rem', lineHeight: '18px', whiteSpace: 'pre-wrap', wordBreak: 'break-word', minWidth: 0, cursor: 'text' },
        title: '双击编辑',
        onDoubleClick: function() { setLogEdit(i) },
        children: l.text,
      }),
    ]}, l.date + '-' + i)
  })
  ...
  function Sec(props) {
    return jsxs('div', { style: { marginBottom: '16px' }, children: [
      jsx('div', { className: 'text-[0.6875rem] font-medium uppercase tracking-wide', style: { color: MUT, marginBottom: '6px' }, children: props.label }),
      props.children,
    ]})
  }
  ...
  return jsxs('div', {
    ...
    style: { background: 'rgba(23,23,28,0.25)', animation: 'pwFadeIn 0.25s ease' },
    onClick: function() { R.setDw(null) },
    children: [
      jsxs('div', {
        className: 'absolute top-0 right-0 bottom-0 bg-white',
        style: { width: 800, maxWidth: '92vw', boxShadow: '0 0 0 1px #e0e2e5, -8px 0 28px rgba(16,24,40,0.06)', borderRadius: '12px 0 0 12px', display: 'flex', flexDirection: 'column', animation: 'pwDrawerIn 0.3s cubic-bezier(0.16,1,0.3,1)' },
        onClick: function(ev) { ev.stopPropagation() },
        children: [
          // ─── 全宽头部（贯穿左右面板） ───
          jsxs('div', { style: { padding: '20px 20px 0 20px', flexShrink: 0 }, children: [
            // 顶部
            jsxs('div', { className: 'flex items-center justify-between', style: { marginBottom: '12px' }, children: [
              jsx('span', { className: 'text-[0.75rem] cursor-pointer transition-colors', style: { color: MUT }, onClick: function() { R.setDw(null) }, children: '← ' + (R.sel || 'Projects') }),
              jsx('span', { className: 'flex items-center justify-center w-6 h-6 rounded-md cursor-pointer transition-colors hover:bg-[#f7f8f9]', style: { color: MUT }, onClick: function() { R.setDw(null) }, children: jsx(Codicon, { name: 'close', className: 'text-[0.875rem]' }) }),
            ]}),
            // 标题 + 操作按钮
            jsxs('div', { className: 'flex items-center gap-2', style: { marginBottom: '8px' }, children: [
              jsxs('div', { className: 'flex-1 min-w-0 flex items-center gap-1.5', children: [
                editingTitle
                  ? jsx('input', {
                      id: 'edtTitle',
                      defaultValue: t.title,
                      autoFocus: true,
                      style: { flex: 1, minWidth: 0, fontSize: '1rem', fontWeight: 600, color: INK, border: '1px solid ' + ACC, borderRadius: '6px', padding: '2px 6px', outline: 'none', background: '#fff', height: '28px' },
                      onKeyDown: function(ev) {
                        if (ev.key === 'Enter') { var v = ev.currentTarget.value.trim(); if (v) R.doRenameTitle(t, v); setEditingTitle(false) }
                        if (ev.key === 'Escape') setEditingTitle(false)
                      },
                      onBlur: function(ev) { var v = ev.currentTarget.value.trim(); if (v && v !== t.title) R.doRenameTitle(t, v); setEditingTitle(false) },
                    })
                  : jsx('h2', { className: 'text-[1rem] font-semibold leading-tight truncate cursor-text', style: { color: INK }, onClick: function() { setEditingTitle(true) }, title: '点击编辑标题', children: t.title }),
                t.repeat_mode ? jsx('span', { title: '重复周期任务：' + repLabel(t), style: { color: MUT, flexShrink: 0, fontSize: '11px', fontWeight: 400 }, children: '🔁 ' + repLabel(t) }) : null,
              ]}),
              // 编辑标题按钮（小铅笔）
              !editingTitle && jsx('span', { className: 'flex items-center justify-center w-6 h-6 rounded-md cursor-pointer transition-colors hover:bg-[#f0f1f2]', style: { color: MUT }, onClick: function() { setEditingTitle(true) }, title: '编辑标题', children: jsx(Codicon, { name: 'edit', className: 'text-[0.8125rem]' }) }),
              // Review 状态专用按钮组
              t.status === 'Review' && jsxs('div', { className: 'flex items-center gap-1.5', children: [
                jsx(Btn, { onClick: function() { R.doReviewPass(t) }, children: [jsx(Codicon, { name: 'check', className: 'text-[0.75rem]' }), '验收通过'], gap: true }),
                jsx('span', { className: 'inline-flex items-center gap-1 text-[0.6875rem] font-medium px-2.5 cursor-pointer select-none transition-all hover:opacity-85', style: { background: ACC, color: '#fff', borderRadius: '8px', height: '24px', lineHeight: '24px' }, onClick: function() { R.doReviewResume(t) }, children: [jsx(Codicon, { name: 'sync', className: 'text-[0.75rem]' }), '人为介入处理'] }),
              ]}),
              t.status !== 'Review' && jsx(Btn, { onClick: function() { R.doHandleTask(t) }, children: '去处理' }),
              // 删除按钮
              jsx('span', { className: 'inline-flex items-center gap-1 text-[0.6875rem] font-medium px-2.5 cursor-pointer select-none transition-all hover:opacity-85', style: { background: DANGER_T, color: DANGER, borderRadius: '8px', height: '24px', lineHeight: '24px' }, onClick: function() { R.doDeleteTask(t) }, children: [jsx(Codicon, { name: 'trash', className: 'text-[0.75rem]' }), '删除'] }),
            ]}),
            // 属性行 1：状态、优先级、处理人、完成
            jsxs('div', { className: 'flex items-center gap-4', style: { marginBottom: '4px' }, children: [
              // 状态
              jsxs('div', { className: 'flex items-center gap-1.5', children: [
                jsx('span', { className: 'text-[0.6875rem]', style: { color: FAINT, whiteSpace: 'nowrap' }, children: '状态' }),
                jsx('select', {
                  className: 'text-[0.75rem] font-medium border-none outline-none cursor-pointer',
                  style: { color: (SBGC[t.status] || { color: MUT }).color, height: '24px', background: 'transparent' },
                  value: t.status,
              ...
              // 优先级
              jsxs('div', { className: 'flex items-center gap-1.5', children: [
                jsx('span', { className: 'text-[0.6875rem]', style: { color: FAINT, whiteSpace: 'nowrap' }, children: '优先级' }),
                jsx('select', {
                  className: 'text-[0.75rem] font-medium border-none outline-none cursor-pointer',
                  style: { color: (PBGC[t.priority] || { color: MUT }).color, height: '24px', background: 'transparent' },
                  value: t.priority,
                  onChange: function(e) { R.doSetField(t, 'priority', e.target.value) },
                  children: [['p0','P0'],['p1','P1'],['p2','P2']].map(function(o) { return jsx('option', { value: o[0], children: o[1] }, o[0]) }),
                }),
              ]}),
              // 处理人
              jsxs('div', { className: 'flex items-center gap-1.5', children: [
                jsx('span', { className: 'text-[0.6875rem]', style: { color: FAINT, whiteSpace: 'nowrap' }, children: '处理人' }),
              ...
              t.complete
                ? jsxs('div', { className: 'flex items-center gap-1.5', children: [
                    jsx('span', { className: 'text-[0.6875rem]', style: { color: FAINT, whiteSpace: 'nowrap' }, children: '完成' }),
                    jsx('span', { className: 'text-[0.75rem]', style: { color: MUT, height: '24px', lineHeight: '24px' }, children: dL(t.complete) }),
                  ]})
                : jsxs('div', { className: 'flex items-center gap-1.5', children: [
                    jsx('span', { className: 'text-[0.6875rem]', style: { color: FAINT, whiteSpace: 'nowrap' }, children: '完成' }),
                    jsx('input', { type: 'date', defaultValue: '', style: { color: MUT, border: 'none', background: 'transparent', outline: 'none', fontSize: '0.75rem', height: '24px', cursor: 'pointer' }, onChange: function(e) { R.doSetField(t, 'complete', e.target.value); t.complete = e.target.value } }),
                  ]}),
            ]}),
            // 属性行 2：开始、截止、记录
            jsxs('div', { className: 'flex items-center gap-4', style: { color: MUT }, children: [
              jsxs('div', { className: 'flex items-center gap-1.5', children: [
                jsx('span', { className: 'text-[0.6875rem] ', style: { color: FAINT, whiteSpace: 'nowrap' }, children: '开始' }),
                jsx('input', { type: 'date', defaultValue: t.start || '', style: { color: MUT, border: 'none', background: 'transparent', outline: 'none', fontSize: '0.75rem', height: '24px', cursor: 'pointer' }, onChange: function(e) { R.doSetField(t, 'start', e.target.value) } }),
              ]}),
              jsxs('div', { className: 'flex items-center gap-1.5', children: [
                jsx('span', { className: 'text-[0.6875rem] ', style: { color: FAINT, whiteSpace: 'nowrap' }, children: '截止' }),
                jsx('input', { type: 'date', defaultValue: t.due || '', style: { color: MUT, border: 'none', background: 'transparent', outline: 'none', fontSize: '0.75rem', height: '24px', cursor: 'pointer' }, onChange: function(e) { R.doSetField(t, 'due', e.target.value) } }),
              ]}),
              jsxs('div', { className: 'flex items-center gap-1.5', children: [
                jsx('span', { className: 'text-[0.6875rem] ', style: { color: FAINT, whiteSpace: 'nowrap' }, children: '记录' }),
                jsx('span', { className: 'text-[0.75rem]', style: { color: MUT, height: '24px', lineHeight: '24px' }, children: (t.logs || []).length + ' 条' }),
              ]}),
            ]}),
          ]}),
          // 分割线
          jsx('div', { style: { height: '1px', background: LINE, margin: '12px 20px', flexShrink: 0 } }),
          // ─── 左右分栏内容区 ───
          jsxs('div', { style: { display: 'flex', flex: 1, overflow: 'hidden' }, children: [
            // ─── 左侧面板：任务详情、目标、验收标准 ───
            jsx('div', { style: { width: '50%', borderRight: '1px solid ' + LINE, overflowY: 'auto' }, children: jsxs('div', { style: { padding: '0 20px 20px 20px' }, children: [
              jsxs('div', { className: 'flex items-center justify-between', style: { marginBottom: '8px' }, children: [
                jsx('span', { className: 'text-[0.6875rem] font-medium', style: { color: MUT }, children: '任务详情' }),
                !tdEditing && jsx('span', {
                  className: 'text-[0.625rem] cursor-pointer transition-colors hover:opacity-70',
                  style: { color: FAINT },
                  onClick: startTdEdit,
              ...
              tdEditing
                ? jsxs('div', { className: 'flex flex-col gap-2', children: [
                    jsx(Input, { as: 'textarea', id: 'tdInput', defaultValue: tdVal, border: LINE2, minHeight: '240px' }),
                    jsxs('div', { className: 'flex gap-2', children: [
                      jsx(Btn, { onClick: saveTd, children: '保存' }),
                      jsx('span', { className: 'inline-flex items-center text-[0.6875rem] px-2.5 cursor-pointer select-none transition-colors hover:bg-[#f4f5f6] border border-[#e5e5e5]', style: { color: BODY, borderRadius: '8px', height: '24px', lineHeight: '24px' }, onClick: function() { setTdEditing(false) }, children: '取消' }),
                    ]}),
                  ]})
                : jsx('div', { className: 'rounded-[8px]', style: { background: FIELD, padding: '12px', color: BODY, fontSize: '0.75rem', lineHeight: '1.7', whiteSpace: 'pre-wrap', wordBreak: 'break-word' }, children: t.task_detail || '—' }),// 目标（Context Cards 信息块）
              jsx(Sec, { label: '目标', children: jsx('div', { className: 'rounded-[8px]', style: { background: FIELD, padding: '10px 12px', color: BODY, fontSize: '0.75rem', lineHeight: '1.7', whiteSpace: 'pre-wrap', wordBreak: 'break-word' }, children: t.goal || '—' }) }),
              // 验收标准（TodoList 风格：完成计数 + 进度条 + 圆形状态标记分条）
              jsx(Sec, { label: '验收标准', children: acItems.length > 0 ? jsx('div', { className: 'overflow-hidden rounded-[8px]', style: { background: '#fff', boxShadow: SH_CARD }, children: [
                jsxs('div', { style: { padding: '10px 12px', borderBottom: '1px solid ' + LINE }, children: [
                  jsxs('div', { className: 'flex items-baseline justify-between', style: { marginBottom: '7px' }, children: [
                    jsx('span', { className: 'text-[0.6875rem] font-medium', style: { color: BODY }, children: acDone + ' / ' + acLen + ' 条通过' }),
                    jsx('span', { className: 'text-[0.6875rem] font-medium tabular-nums', style: { color: acDone === acLen ? GREEN : MUT }, children: Math.round(acLen ? acDone / acLen * 100 : 0) + '%' }),
                  ]}),
                  jsx('div', { style: { height: '4px', borderRadius: '2px', background: FIELD, overflow: 'hidden' }, children: jsx('div', { style: { height: '100%', borderRadius: '2px', background: acDone === acLen ? GREEN : ACC, width: Math.round(acLen ? acDone / acLen * 100 : 0) + '%', transition: 'width 0.3s ease' } }) }),
                ]}),
                acItems,
              ] }) : jsx('span', { className: 'text-[0.75rem]', style: { color: FAINT }, children: '—' }) }),
              
            ]}) }),
// ─── 右侧面板：关联会话、推进记录 ───
            jsx('div', { style: { width: '50%', overflowY: 'auto' }, children: jsxs('div', { style: { padding: '0 20px 20px 20px' }, children: [
// 关联会话
              (function() {
                var sessContent
                if (!sessLoaded) {
                  sessContent = jsx('span', { className: 'text-[0.75rem]', style: { color: FAINT }, children: '加载中…' })
                } else if (!taskSess.length) {
                  sessContent = jsx('span', { className: 'text-[0.75rem]', style: { color: FAINT }, children: '暂无关联会话' })
                } else {
                  sessContent = taskSess.map(function(s) {
                    var src = s.source || ''
                    var bg = INSET, fg = BODY
                    if (src === 'kanban') { bg = PURPLE_T; fg = PURPLE }
                    else if (src === 'cron') { bg = ORANGE_T; fg = ORANGE }
                    else if (['desktop','cli','webui','tui','weixin','acp'].indexOf(src) >= 0) { bg = ACCS; fg = ACC }
                    var dt = s.last_activity_at ? new Date((s.last_activity_at > 10**12 ? s.last_activity_at / 1000 : s.last_activity_at) * 1000).toLocaleString('zh-CN', { month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' }) : ''
                    return jsxs('div', { className: 'cursor-pointer hover:bg-[#f4f5f6] rounded py-1', onClick: function() { host.request('session.resume', { session_id: s.id }).then(function() { host.navigate('/' + encodeURIComponent(s.id)) }).catch(function(e) { console.error('[pw] session.resume failed:', e) }) }, children: [
                      jsx('div', { className: 'text-[0.75rem] truncate', style: { color: BODY }, children: s.title || '(无标题)' }),
                      jsxs('div', { className: 'flex items-center gap-1.5 mt-0.5', children: [
                        src ? jsx('span', { className: 'text-[0.5625rem] font-medium px-1.5 rounded-full', style: { background: bg, color: fg, lineHeight: '18px' }, children: src }) : null,
                        dt ? jsx('span', { className: 'text-[0.625rem]', style: { color: FAINT }, children: dt }) : null,
                      ]}),
                    ]}, s.id)
                  })
                }
                return jsx(Sec, { label: '关联会话', children: jsx('div', { className: 'overflow-hidden rounded-[8px]', style: { background: '#fff', boxShadow: SH_CARD, padding: '6px 10px' }, children: jsxs('div', { className: 'flex flex-col gap-1', children: sessContent }) }) })
              })(),
              jsx(Sec, { label: '推进记录', children: logItems.length > 0 ? jsx('div', { className: 'overflow-hidden rounded-[8px]', style: { background: '#fff', boxShadow: SH_CARD, padding: '6px 10px' }, children: logItems }) : jsx('span', { className: 'text-[0.75rem]', style: { color: FAINT }, children: '暂无记录' }) }),
              jsxs('div', { className: 'flex gap-2', children: [
                jsx(Input, { id: 'logInput', placeholder: '添加推进记录…', onKeyDown: function(ev) { if (ev.key === 'Enter') R.doAddLog(t) } }),
...
// ─── Modal (新建项目/任务，Linear 风格) ────────────────────
function Modal(R) {
  console.log('[pw] Modal render, md=', R.md)
  if (!R.md) return null
    ...
    open && filtered.length > 0 && jsxs('div', {
      ...
      children: filtered.map(function(h) {
        return jsx('div', {
          ...
          onMouseEnter: function(e) { e.target.style.background = HOV },
          onMouseLeave: function(e) { e.target.style.background = h === val ? HOV : 'transparent' },
          children: h,
        }, h)
      }),
    }),
