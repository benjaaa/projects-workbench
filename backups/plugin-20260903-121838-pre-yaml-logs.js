          ...
          runSpec({ op: 'set_property', path: t.path, field: 'status', value: 'Review' }).then(function() {
            ...
            runSpec({ op: 'kanban_worker_session', task_id: t.kanban_task_id, board: 'default' }).then(function(wr) {
              if (wr && wr.worker_session_id) {
                ...
                var logDate = p2(now.getMonth() + 1) + '-' + p2(now.getDate())
...
function TaskDetailPage(R) {
  ...
  var tlData = useState(null), timeline = tlData[0], setTimeline = tlData[1]
  ...
  useEffect(function() {
    ...
    if (ids.length) {
      ...
      // 加载 session 时间线分片
      runSpec({ op: 'session_timeline', ids: ids.join(',') }).then(function(r) {
        setTimeline(r.timeline || []); setSessTlLoaded(true)
      }).catch(function(e) { console.error('[pw] session_timeline failed:', e); setSessTlLoaded(true) })
    } else {
  ...
  // ── 统一时间线：session 分片 + 人工记录合并，按时间倒序 ──
  var tlEntries = []
  // session 分片
  if (timeline) {
    timeline.forEach(function(sess) {
      (sess.days || []).forEach(function(day) {
        tlEntries.push({
          type: 'session', date: day.date, session_id: sess.session_id,
          title: sess.title, source: sess.source,
          rounds: day.rounds, last_human: day.last_human, last_agent: day.last_agent,
        })
      })
    })
  }
      ...
      jsxs('div', { style: { display: 'flex', gap: '36px', alignItems: 'flex-start' }, children: [
        ...
        jsxs('div', { style: { flex: 1, minWidth: 0 }, children: [
          ...
          jsxs('div', { style: { display: 'flex', gap: '6px', marginBottom: '14px', width: '100%' }, children: [
            jsx('div', { style: { flex: 1, minWidth: 0 }, children: jsx(Input, { id: 'logInput', placeholder: '添加推进记录…', classNameExtra: ' w-full', onKeyDown: function(ev) { if (ev.key === 'Enter') R.doAddLog(t) } }) }),
            ...
            tlEntries.length === 0 && sessTlLoaded
              ...
              : tlEntries.map(function(entry, ei) {
                  if (entry.type === 'session') {
                    var srcColor = entry.source === 'kanban' ? PURPLE : ACC
                    var srcBg = entry.source === 'kanban' ? PURPLE_T : ACCS
                    return jsxs('div', { style: { display: 'flex', gap: '12px', padding: '7px 0', position: 'relative' }, children: [
                      jsx('span', { style: { width: '11px', height: '11px', borderRadius: 99, flexShrink: 0, marginTop: '4px', position: 'relative', zIndex: 1, border: '2px solid #fff', background: entry.source === 'kanban' ? PURPLE : ACC } }),
                      jsxs('div', { style: { flex: 1, minWidth: 0 }, children: [
                        jsxs('div', { style: { display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '3px' }, children: [
                          jsx('span', { style: { fontSize: '10px', color: MUT, fontFamily: 'monospace', flexShrink: 0 }, children: entry.date }),
                          jsx('span', { style: { fontSize: '9px', fontWeight: 500, padding: '0 5px', borderRadius: 99, lineHeight: '15px', background: srcBg, color: srcColor, flexShrink: 0 }, children: entry.source || 'session' }),
                          jsx('span', { style: { fontSize: '12px', color: MUT, flexShrink: 0 }, children: entry.title }),
                          jsx('span', {
                            style: { fontSize: '10px', color: ACC, cursor: 'pointer', marginLeft: 'auto', flexShrink: 0, fontWeight: 500 },
                            onClick: function() { doReviewResumeSess(entry.session_id) },
                            children: '继续会话 →',
                          }),
                        ]}),
                        jsxs('div', { style: { background: INSET, borderRadius: '5px', padding: '10px 12px', marginTop: '4px' }, children: [
                          entry.last_human ? jsxs('div', { style: { marginBottom: '6px' }, children: [
                            jsx('div', { style: { fontSize: '10px', fontWeight: 600, color: ACC, textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: '1px' }, children: 'Human' }),
                            jsx('div', { style: { fontSize: '12px', color: BODY, lineHeight: 1.6 }, children: entry.last_human }),
                          ] }) : null,
                          entry.last_agent ? jsxs('div', { children: [
                            jsx('div', { style: { fontSize: '10px', fontWeight: 600, color: PURPLE, textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: '1px' }, children: 'Agent' }),
                            jsx('div', { style: { fontSize: '12px', color: BODY, lineHeight: 1.6 }, children: entry.last_agent }),
                          ] }) : null,
                          entry.rounds > 1 && jsx('div', { style: { fontSize: '10px', color: FAINT, marginTop: '6px' }, children: '当天共 ' + entry.rounds + ' 轮对话' }),
                        ]}),
                      ]}),
                    ] }, 's' + entry.session_id + entry.date)
                  }
                  // 人工记录
                  return jsxs('div', { style: { display: 'flex', gap: '12px', padding: '7px 0', position: 'relative' }, children: [
