...
function useReducer(initialView) {
          ...
          runSpec({ op: 'set_property', path: t.path, field: 'status', value: 'Review' }).then(function() {
            ...
            runSpec({ op: 'kanban_worker_session', task_id: t.kanban_task_id, board: 'default' }).then(function(wr) {
              if (wr && wr.worker_session_id) {
                ...
                var entryId = String(now.getFullYear()) + p2(now.getMonth() + 1) + p2(now.getDate()) + '_' + p2(now.getHours()) + p2(now.getMinutes()) + p2(now.getSeconds())
  ...
  function doCreateSess(pjn, task) {
    // 与 doHandleTask 一致：把任务文件带回 chat 首页，由 doSend（工作正常的链路）创建 session
    // 不走 session.create——plugin 直接调 session.create 无法建立 owner hint，dispatcher 找不到路由
    var projDir = pjn.dir || pjn.title
    var cwd = VAULT + '/' + PROOT + '/' + projDir
    if (task) {
      // 任务级会话：带任务文件
      window.__pwPreloadCtx = {
        type: 'issue',
        name: task.title || '任务',
        dir: projDir,
        path: task.path,
        project: task.project || projDir,
        goal: task.goal || '',
        task_detail: task.task_detail || '',
        acceptance_criteria: task.acceptance_criteria || []
      }
    } else {
      // 项目级会话：只带项目（type: 'project' 显示项目 chip，不带文案）
      window.__pwPreloadCtx = {
        type: 'project',
        name: pjn.title || projDir,
        dir: projDir,
        path: pjn.path || ''
      }
    }
...
function TaskDetailPage(R) {
  ...
  var ey = useState(null), editingYaml = ey[0], setEditingYaml = ey[1]
  ...
  function saveYamlEdit(entry) {
    var inp = document.getElementById('yamlEditInput')
    if (!inp) return
    var newSummary = inp.value.trim()
    if (!newSummary) { setEditingYaml(null); return }
    // 重建 YAML 条目（保留原有字段，只替换 summary）
    ...
    if (entry.sessions.length > 0) {
      ...
      entry.sessions.forEach(function(s) {
        yamlText += '    - id: ' + s.id + '\n'
        yamlText += '      source: ' + (s.source || 'desktop') + '\n'
      })
    }
    if (entry.outputs.length > 0) { yamlText += '  outputs:\n'; entry.outputs.forEach(function(o) { yamlText += '    - ' + o + '\n' }) }
    if (entry.decisions.length > 0) { yamlText += '  decisions:\n'; entry.decisions.forEach(function(d) { yamlText += '    - desc: ' + d.desc + '\n      by: ' + d.by + '\n' }) }
    if (entry.risks.length > 0) { yamlText += '  risks:\n'; entry.risks.forEach(function(r) { yamlText += '    - ' + r + '\n' }) }
    if (entry.pending.length > 0) { yamlText += '  pending:\n'; entry.pending.forEach(function(p) { yamlText += '    - ' + p + '\n' }) }
    // 乐观更新
    var updated = Object.assign({}, t, { logs_yaml: t.logs_yaml.replace(
              ...
              : tlEntries.map(function(entry, ei) {
                  ...
                  if (entry.type === 'yaml') {
                    ...
                    return jsxs('div', { style: { display: 'flex', gap: '12px', padding: '7px 0', position: 'relative' }, children: [
                      ...
                      jsxs('div', { style: { flex: 1, minWidth: 0 }, children: [
                        jsxs('div', { style: { display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '4px', flexWrap: 'wrap' }, children: [
                          jsx('span', { style: { fontSize: '12px', color: MUT, fontFamily: 'monospace', flexShrink: 0 }, children: entry.raw_date || entry.date }),
                          jsx('span', { style: { fontSize: '11px', fontWeight: 500, padding: '0 6px', borderRadius: 99, lineHeight: '16px', background: typeBg, color: typeColor, flexShrink: 0 }, children: typeLabel }),
                          // review 周期展示（仅 review 类型）
                          entry.logType === 'review' && entry.window && jsx('span', { style: { fontSize: '11px', color: FAINT, flexShrink: 0 }, children: entry.window }),
                          ...
                          // 编辑按钮（仅 YAML 条目有 id 时可用）
                          entry.entryId && jsx('span', {
                            style: { fontSize: '10px', color: MUT, cursor: 'pointer', flexShrink: 0, marginLeft: '4px' },
                            title: '编辑此记录',
                            onClick: function() { setEditingYaml(entry.entryId) },
                            children: '✎',
                          }),
                        ]}),
                        editingYaml === entry.entryId
                          ? jsxs('div', { style: { display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '4px' }, children: [
                              jsx('input', {
                                id: 'yamlEditInput', defaultValue: entry.summary,
                                ...
                                autoFocus: true,
                                onKeyDown: function(ev) { if (ev.key === 'Enter') saveYamlEdit(entry) },
                              }),
                        ...
                        (entry.outputs.length > 0 || entry.decisions.length > 0 || entry.risks.length > 0 || entry.pending.length > 0) && jsxs('div', { style: { background: INSET, borderRadius: '5px', padding: '12px 14px', marginTop: '6px' }, children: [
                          entry.outputs.length > 0 && jsxs('div', { style: { marginBottom: '8px' }, children: [
                            ...
                            entry.outputs.map(function(d, di) { return jsx('div', { style: { fontSize: '12px', color: INK, lineHeight: 1.75, paddingLeft: '10px' }, children: '· ' + d }, 'd' + di) }),
                          ]}),
                          entry.decisions.length > 0 && jsxs('div', { style: { marginBottom: '8px' }, children: [
                            jsx('div', { style: { fontSize: '11px', fontWeight: 600, color: ACC, textTransform: 'uppercase', letterSpacing: 0, marginBottom: '3px' }, children: '关键决策' }),
                            entry.decisions.map(function(d, di) { return jsxs('div', { style: { fontSize: '12px', color: INK, lineHeight: 1.75, paddingLeft: '10px' }, children: ['· ' + d.desc, d.by === 'ben' ? jsx('span', { style: { fontSize: '12px', color: ACC, marginLeft: '4px' }, children: '(Ben)' }) : null] }, 'dc' + di) }),
                          ]}),
                          entry.risks.length > 0 && jsxs('div', { style: { marginBottom: '8px' }, children: [
                            jsx('div', { style: { fontSize: '11px', fontWeight: 600, color: ORANGE, textTransform: 'uppercase', letterSpacing: 0, marginBottom: '3px' }, children: '⚠ 注意' }),
                            entry.risks.map(function(r, ri) { return jsx('div', { style: { fontSize: '12px', color: INK, lineHeight: 1.75, paddingLeft: '10px' }, children: '· ' + r }, 'r' + ri) }),
                          ]}),
                          entry.pending.length > 0 && jsxs('div', { children: [
                            jsx('div', { style: { fontSize: '11px', fontWeight: 600, color: MUT, textTransform: 'uppercase', letterSpacing: 0, marginBottom: '3px' }, children: '遗留' }),
                            entry.pending.map(function(p, pi) { return jsx('div', { style: { fontSize: '12px', color: INK, lineHeight: 1.75, paddingLeft: '10px' }, children: '· ' + p }, 'p' + pi) }),
                          ]}),
                        ]}),
                      ]}),
                    ] }, 'y' + ei)
                  ...
                  return jsxs('div', { style: { display: 'flex', gap: '12px', padding: '7px 0', position: 'relative' }, children: [
                    ...
                    jsxs('div', { style: { flex: 1, minWidth: 0 }, children: [
                      ...
                      logEdit === entry.idx
                        ...
                        : jsx('div', {
                            style: { fontSize: '12px', color: INK, lineHeight: 1.75, cursor: 'text' },
                            title: '双击编辑',
